...
import { Pass, Effect, RenderPass, Selection, NormalPass } from 'postprocessing';
import { DataTexture, RGBAFormat, FloatType, ShaderChunk, ShaderLib, UniformsUtils, WebGLMultipleRenderTargets, ShaderMaterial, GLSL3, NoBlending, Uniform, Vector2, Matrix4, Vector3, Clock, Quaternion, LinearFilter, HalfFloatType, UnsignedByteType, RGBADepthPacking, BasicDepthPacking, NearestFilter, WebGLRenderTarget, ClampToEdgeWrapping, LinearMipMapLinearFilter, EquirectangularReflectionMapping, Color, Matrix3, TangentSpaceNormalMap, RepeatWrapping, RedFormat, Source, MeshDepthMaterial, BackSide, TextureLoader, NoColorSpace, Texture, NoToneMapping, PerspectiveCamera, SRGBColorSpace, DepthTexture, FramebufferTexture } from 'three';
...
class DenoisePass extends Pass {
  ...
  setTextures(textures) {
    ...
    let finalFragmentShader = definitions + fragmentShader$4.replace("#include <denoiseCustomComposeShaderFunctions>", this.options.denoiseCustomComposeShaderFunctions).replace("#include <denoiseCustomComposeShader>", this.options.denoiseCustomComposeShader).replace("#include <outputShader>", outputShader).replace("#include <gbuffer_packing>", gbuffer_packing).replaceAll("textureCount", this.textures.length).replaceAll("momentTextureCount", Math.min(this.textures.length, 2));
    finalFragmentShader = unrollLoops(finalFragmentShader);
    const matches = finalFragmentShader.matchAll(/inputTexture\[\s*[0-9]+\s*]/g);
...
class SSGIMaterial extends ShaderMaterial {
  constructor() {
    super({
      ...
      fragmentShader: fragmentShader$3.replace("#include <ssgi_utils>", ssgi_utils).replace("#include <gbuffer_packing>", gbuffer_packing).replace("#include <sampleBlueNoise>", sampleBlueNoise),
      vertexShader,
      blending: NoBlending,
      depthWrite: false,
      depthTest: false,
