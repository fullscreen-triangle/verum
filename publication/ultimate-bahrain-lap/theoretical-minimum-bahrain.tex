...
successive constraints. Formula One, in contrast, lives at the intersection of
at least five constraints that are \emph{all} simultaneously active for some
...
simulation}: a high-fidelity vehicle model (multibody suspension, Pacejka
tire model, CFD-derived aeromap, transient thermal model) is coupled to a
lap-long driver model (either a rule-based racing-line solver or a
...
simulation minimum is a moving target; a theoretical minimum is not.
...
The Bahrain International Circuit, located in Sakhir approximately 30\,km south
of Manama, was designed by Hermann Tilke and opened in 2004. The Grand Prix
...
GPS traces, satellite imagery, and onboard telemetry fits. Where onboard data
...
\caption{Circuit and speed profile. (A) Two-dimensional layout of the Bahrain
International Circuit with corner markers colour-coded by radius. (B) Predicted
speed trace vs.\ lap distance. (C) Three-dimensional speed surface over
(distance, lap-number) showing theoretical vs.\ simulated stochastic laps.
(D) Corner radius bar chart for T1--T15.}
\label{fig:panel1}
\end{figure*}
...
reduction, justified because over a lap the lateral dynamics are slaved to the
longitudinal dynamics via the friction circle (Section~\ref{sec:tire}).

The minimum mass under 2023 regulations is
\[
...
coefficient. For constant-power propulsion,
\[
...
\begin{equation}
  ...
  \label{eq:hk-eom}
\end{equation}
which is the direct Formula One analogue of the Hill--Keller sprint equation
originally derived for human athletes (replacing a constant propulsive force
...
\begin{equation}
  ...
  \label{eq:geff}
\end{equation}
Here $C_L A$ is the lift-area product; for a 2023 Bahrain low-downforce setup
we adopt $C_L A = 4.0~\mathrm{m}^2$.
...
Mureika's sprinting-curve model, is
\begin{equation}
  v_{\mathrm{eff}}(R, t)
...
\caption{Constraint boundaries. (A) Maximum lateral acceleration vs.\ corner
radius (friction ellipse with downforce). (B) Terminal velocity vs.\ effective
drag coefficient. (C) Three-dimensional surface of minimum lap time over
ICE power and tire $\mu$. (D) Single-parameter sensitivities
(±10\%).}\label{fig:panel3}
\end{figure*}
...
This is a generalised Kuramoto model on a hierarchy.
...
\[
  ...
  = 2.5~\mathrm{kg}.
\]
With $Q_f = 43$~MJ/kg, the thermal fuel energy per lap is
\begin{equation}
...
Regulation caps MGU-K deployment from the Energy Store at 4~MJ per lap:
\begin{equation}
  E_{\mathrm{MGU\text{-}K}}^{\mathrm{lap}} = 4~\mathrm{MJ}.
\end{equation}

\paragraph{MGU-H harvested.}
The MGU-H feeds the Energy Store and the MGU-K with turbine-derived energy,
uncapped by regulation. In practice, harvestable energy is bounded by exhaust
...
$\int P\,dt \le E_{\max}$ gives the Lagrangian
...
A standard sensitivity used by race engineers is
$\partial T / \partial m_f \approx +0.03$~s per kg of fuel in Bahrain. This
...
\caption{Energy and power budget. (A) Instantaneous propulsive and dissipative
power vs.\ distance (ICE, MGU-K, drag). (B) Cumulative energy curves (fuel,
ERS deployed, ERS harvested). (C) Three-dimensional energy-state trajectory
over (battery SOC, fuel remaining, distance). (D) Decomposition of lap energy
into kinetic, aerodynamic, tire, and braking contributions.}
\label{fig:panel2}
\end{figure*}
...
tire is well-described by a Pacejka ``Magic Formula''
\begin{align}
...
For 2023 soft-compound Pirelli C4 at the optimal tread temperature
...
temperature. A simplified thermal energy balance,
\begin{equation}
...
carbon--carbon disc system. Suspension resonance of 3--5~Hz further constrains
the maximum lateral jerk to
...
conditions $\lambda_\star(s)\, g_\star(s) = 0$. If no constraint were active,
...
scheme (details in the computational script
...
\caption{Historical progression and extreme-value analysis. (A) Bahrain
pole-position time vs.\ year with regression trend. (B) Distribution of
fastest race laps with Gumbel fit. (C) Three-dimensional speed traces
across five eras. (D) 20-year projection of expected record.}
\label{fig:panel4}
\end{figure*}
...
(i.i.d.\ lap-time errors with exponentially-bounded tails) the distribution
of $-T_{(1)}^{(n)}$ is Gumbel:
\begin{equation}
...
Fitting Eq.~\eqref{eq:gumbel} by maximum likelihood to the full set of
Bahrain pole times in Table~\ref{tab:history} (a pooled sample across
...
Sobol first-order indices estimated from the Monte Carlo ensemble (based on
partial-derivative weightings at reference):
\begin{align*}
...
the practical message is the same: tire friction is the dominant lever.

\begin{figure*}[!t]
\centering
\includegraphics[width=\textwidth]{figures/panel_5_validation.png}
\caption{Monte Carlo validation. (A) Distribution of predicted lap times
with mean, 95\% CI, and observed 2023 record. (B) Per-corner predicted
vs.\ actual time residuals. (C) Three-dimensional parameter-correlation
surface. (D) Convergence of numerical solver.}
\label{fig:panel5}
\end{figure*}
...
first-principles physical components.
...
($S_\mu = 0.48$) and downforce ($S_{C_LA} = 0.23$) dominate. Regulation
