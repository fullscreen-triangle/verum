...
% Title
% ============================================================================
\title{\textbf{Philharmonic: Complete Formula One Vehicle State Reconstruction\\
from Partial Telemetry via Oscillatory Circuit Graph\\
...
\subsection{Our approach: oscillatory circuit graph trajectory completion}
...
\begin{figure}[!htbp]
    ...
    \caption{The Formula One car modelled as a 20-node oscillatory circuit graph. (A) Conductance-weighted adjacency matrix showing mechanical, thermal, electrical, and aerodynamic couplings. (B) Characteristic oscillatory frequencies of all 20 subsystems spanning four orders of magnitude (1--2000\,Hz). (C) Three-dimensional embedding with frequency, categorical depth, and coupling degree as coordinates. (D) Eigenvalue spectrum of the graph Laplacian confirming connectivity ($\lambda_1 = 0$, unique) and quantifying algebraic connectivity via the Fiedler value $\lambda_2 > 0$.}
...
\begin{figure}[!htbp]
    ...
    \caption{State reconstruction from partial telemetry. (A) Engine RPM trace over one lap: observed telemetry (blue) versus circuit graph reconstruction (red, perfectly overlaid). (B) Reconstructed battery state-of-charge variation over one lap showing charge/discharge cycling with standard deviation $\sigma = 24.85$. (C) Three-dimensional scatter plot of observed versus reconstructed states for all 20 nodes (points on diagonal indicate perfect reconstruction). (D) Per-node reconstruction error: observed nodes (blue) show zero error; hidden nodes (gold) show consistency-check residuals below $7.2 \times 10^{-9}$.}
...
\begin{figure}[!htbp]
    ...
    \caption{Fault detection via trajectory escape. (A) Categorical depth of the turbocharger node versus lap number under healthy (blue) and faulty (red, 30\% ICE-turbo conductance reduction injected at lap 15) conditions; anomaly detected 13 laps before fault onset. (B) Per-node deviation between healthy and faulty states: brake nodes show highest sensitivity (1.0), aero and suspension nodes show intermediate sensitivity (0.07--0.09). (C) Three-dimensional trajectory escape: the faulty state (red) diverges from the healthy attractor (blue) in the categorical depth space. (D) Detection timeline showing the cumulative deviation metric crossing the alarm threshold at lap 2.}
    \label{fig:fault}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{Tire degradation tracking over a 30-lap stint. (A) Lap time versus lap number showing degradation from 88.9\,s (fresh tires, lap 1) to 107.0\,s (worn tires, lap 30), an increase of 20.3\%. (B) Tire node categorical depth versus lap, tracking the exponential degradation curve. (C) Three-dimensional tire state surface showing the evolution of tire categorical depth, lap time, and grip level across the stint. (D) Predicted degradation cliff (second derivative peak) versus actual pit stop window; the predicted cliff at lap 5 precedes the actual pit at lap 25, indicating calibration required for the absolute timing but correct curve shape.}
    \label{fig:tires}
\end{figure}
...
\begin{figure}[!htbp]
    ...
    \caption{Racing line extraction via S-entropy analysis. (A) Speed traces for all 8 qualifying laps overlaid, with the fastest lap (\#8) highlighted in red. (B) S-entropy components $(S_k, S_t, S_e)$ along the lap distance for the fastest lap, showing high knowledge entropy $S_k > 0.86$ throughout. (C) Three-dimensional S-entropy trajectory $(S_k, S_t, S_e)$ for the fastest (red) and slowest (blue) qualifying laps, showing the tighter trajectory of the faster lap. (D) Sector-by-sector S-entropy comparison: Sector 1 ($S_k = 0.988$, $S_t = 0.228$) shows highest commitment; Sector 2 ($S_k = 0.975$, $S_t = 0.786$) shows highest complexity.}
