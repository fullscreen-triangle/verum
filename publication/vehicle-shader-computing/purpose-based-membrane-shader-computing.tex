...
\begin{figure}[H]
    ...
    \textbf{Figure 1:} Bounded phase space $\Gamma$ partitioned into shells $n = 1, 2, \ldots, n_{\max}$ with shell capacity $C(n) = 2n^2$.  The S-entropy coordinates $(\Sk, \St, \Se) \in [0,1]^3$ map the discrete partition to the unit cube.  The triple equivalence $\Sosc = \Scat = \Spart$ is indicated by the convergence of three entropy measures at the dominant shell.
    \vspace{2cm}}}
    \caption{Partition structure of bounded phase space.}\label{fig:phase-space-partition}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 2:} The membrane as categorical processor.  (a)~Lipid bilayer with inner leaflet (P-type, negatively charged) and outer leaflet (N-type, zwitterionic) forming the biomolecular P-N junction.  (b)~R-C-L trichotomy: each lipid element simultaneously acts as resistor ($\Sk$-channel), capacitor ($\St$-channel), and inductor ($\Se$-channel).  (c)~Content-addressable memory: query pattern $q$ resonantly excites the nearest stored pattern $p^*$.
    \vspace{2cm}}}
    \caption{Membrane processor architecture.}\label{fig:membrane-processor}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 3:} The Spectral Image Theorem.  Left: bounded oscillatory system $X$ with Koopman eigenvalues $\{\lambda_j\}$.  Centre: Gaussian-windowed frequency-phase kernel generating the spectral image $\mathcal{I}_X(\omega, \phi)$.  Right: the spectral image as a 2D intensity map.  The Universal Reduction Theorem states that $\dpart(X, Y) = \dcv(\mathcal{I}_X, \mathcal{I}_Y)$ exactly.
    \vspace{2cm}}}
    \caption{Spectral imaging of bounded oscillatory systems.}\label{fig:spectral-image}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 4:} The Triple Observation Identity.  A ray marching through the vehicle circuit graph computes optical absorption $\mu_a$, chromatographic retention $\tau \cdot d_S$, and circuit current $G \cdot \Delta\varphi$ at every step $ds$.  By \Cref{thm:triple-observation}, these are three representations of the same partition observable.
    \vspace{2cm}}}
    \caption{Triple Observation in a vehicle circuit graph.}\label{fig:triple-observation}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 5:} The five-pass GPU shader pipeline.  Pass~1: Dual-pixel analysis separates visible and invisible channels.  Pass~2: Holographic back-propagation reconstructs the 3D spectral field.  Pass~3: Triple-observation ray march computes all three observations simultaneously.  Pass~4: Multi-ray interference combines observation paths.  Pass~5: Scalar readback with purpose-injected interpretation.
    \vspace{2cm}}}
    \caption{Five-pass spectral matching pipeline on GPU.}\label{fig:five-pass}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 6:} Purpose injection as low-rank spectral prior.  The base engine parameters $\theta_0$ (universal physics) are modified by a low-rank update $\Delta\theta_{\LoRA} = B \cdot A$ with $\mathrm{rank}(BA) = r \ll d$.  The rank $r$ encodes the dimensionality of domain knowledge.  Different domains (vehicle diagnostics, F1 strategy, atmospheric science) correspond to different $(B, A)$ pairs, all sharing the same $\theta_0$.
    \vspace{2cm}}}
    \caption{Purpose injection architecture.}\label{fig:purpose-injection}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 7:} Scattering puzzle assembly.  Multiple data sources (weather, traffic camera, membrane, OBD-II, exhaust) each provide spectral image fragments.  The scattering puzzle assembles these into a complete spectral image via constrained optimisation.  Visible pixels are directly observed; invisible pixels are inferred from consistency constraints.  Purpose injection guides which fragments receive priority.
    \vspace{2cm}}}
    \caption{Scattering puzzle imaging from heterogeneous sources.}\label{fig:scattering-puzzle}
\end{figure}
...
\begin{figure}[H]
    \centering
    \fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}
    \textbf{Figure 8:} The closed spectral matching loop.  Membrane sensing $\to$ spectral imaging $\to$ GPU matching $\to$ purpose-injected interpretation $\to$ actionable output $\to$ vehicle behaviour modification $\to$ changed spectra $\to$ membrane sensing.  By the OCP Identity, the entire loop is a continuous act of categorical address resolution.
    \vspace{2cm}}}
    \caption{The closed spectral matching loop.}\label{fig:matching-loop}
\end{figure}
