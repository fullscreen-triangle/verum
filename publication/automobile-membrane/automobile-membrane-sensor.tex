...
\begin{figure}[H]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}\textbf{Figure Placeholder}\\\textit{Biological semiconductor band diagram for the membrane. Shows P-type (lipid raft domain) and N-type (fluid membrane) regions, the built-in potential $V_{\mathrm{bi}} = \SI{0.78}{V}$, depletion region at the raft-fluid interface, and carrier concentrations. The ``bandgap'' corresponds to the trans-gauche isomerization barrier $\Delta E \approx 3.5\,\kB T$. Current-voltage characteristic shown as inset, demonstrating rectification ratio $> 42$.}\vspace{2cm}}}
\caption{Biological semiconductor model of the lipid membrane. The P-N junction forms at the boundary between lipid raft domains (P-type, enriched in collective oscillatory modes) and fluid membrane regions (N-type, enriched in individual molecular oscillators). The built-in potential of \SI{0.78}{V} and rectification ratio exceeding 42 are comparable to silicon semiconductor junctions, but the membrane semiconductor operates at room temperature in an aqueous environment with self-healing capability. The effective ``bandgap'' is the \emph{trans-gauche} isomerization barrier of approximately $3.5\,\kB T$.}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}\textbf{Figure Placeholder}\\\textit{Phase-locked $\mathrm{O}_2$ ensemble structure at the membrane surface. Shows $\sim 10^4$ oxygen molecules within a coherence sphere of radius $\xicoh \approx \SI{14}{nm}$. Phase arrows indicate synchronized rotational phases $|\phi_i - \phi_j| < \pi/2$. Color coding shows different environmental parameters encoded in the phase structure: coherence time (temperature), ensemble size (pressure), phase spectrum (chemistry), phase gradient (gravity/acceleration), phase drift (flow), decay rate (viscosity), Zeeman splitting (electromagnetic fields).}\vspace{2cm}}}
\caption{Phase-locked $\mathrm{O}_2$ ensemble at the membrane surface. Approximately $10^4$ oxygen molecules within a coherence sphere of radius $\xicoh \approx \SI{14}{nm}$ maintain synchronized rotational phases through Van der Waals and paramagnetic coupling. The ensemble's phase structure encodes the complete environmental state---temperature, pressure, chemistry, flow, gravity, viscosity, and electromagnetic fields---in a compressed 15-bit representation. This $10^4$-fold reality compression enables the membrane to process environmental information at categorical (individual-molecule) bandwidth rather than ensemble (statistical-averaging) bandwidth.}
\label{fig:o2_ensemble}
\end{figure}
...
\begin{figure}[H]
\centering
\fbox{\parbox{0.85\textwidth}{\centering\vspace{2cm}\textbf{Figure Placeholder}\\\textit{Vehicle membrane coverage topology. Plan view, side view, front view, and rear view of a passenger car showing membrane coverage regions (shaded) and uncovered regions (glass, joints, high-wear areas). The coverage provides $> 4\pi/3$ steradian coverage with no uncovered hemisphere. Numbered regions show distinct partition domains with different $(n, \ell, m, s)$ parameters, corresponding to different surface orientations and environmental exposures.}\vspace{2cm}}}
\caption{Membrane coverage topology for a typical passenger car. The membrane covers approximately \SI{10}{m^2} of the vehicle's \SI{12.4}{m^2} exterior surface, providing greater than $4\pi/3$ steradian coverage with no uncovered hemisphere. Different regions of the membrane develop distinct partition domains (analogous to lipid raft domains in biological membranes) corresponding to their different orientations and environmental exposures---roof regions sample vertical atmospheric gradients, side panels sample horizontal gradients, and underbody regions sample road-surface thermal coupling.}
\label{fig:vehicle_coverage}
\end{figure}
